...
#include <stdio.h>
#include <string.h>	// For stringcpy and memset()
#include <stdlib.h>
#include <sys/ioctl.h>
#include <sys/socket.h>	// for socket()
#include <net/if.h>
#include <netinet/ether.h>	// for arp
...
void send_arp(char* address, char* iName ,char* destIp, short type) {
	...
	int atic = 0; // arp target ip counter
	while (atic < 4) {
	...
	int asic = 0; // arp sender ip counter
	while (asic < 4) {
...
void recv_arp(char * iName) {
	...
	if (ioctl(sockfd, SIOCGIFHWADDR, &if_mac) < 0)
	    printf("SIOCGIFHWADDR");
	printf("Mac has been set\n");
	// Listening for incoming ether net frame from sockfd.
	// We expect to receive here an ARP ethernet frame and
	// we continue until we get an ARP reply.
	while(1) {
		int invalid = 1;
		while(invalid) {
			invalid = 0;
			...
			while ((eh->ether_dhost[0] != ((uint8_t *)&if_mac.ifr_hwaddr.sa_data)[0] || eh->ether_dhost[1] != ((uint8_t *)&if_mac.ifr_hwaddr.sa_data)[1] ||
